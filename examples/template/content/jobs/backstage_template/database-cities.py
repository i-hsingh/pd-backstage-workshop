...
template = """# Backstage Scaffolder Template for Database Backup and Restore via Rundeck
apiVersion: scaffolder.backstage.io/v1beta3
kind: Template
metadata:
  name: rundeck-database-backup-template-{city}
  title: Database Backup and Restore ({citytitle})
  description: Use this template to perform database backup and restore operations for {citytitle} with configurable retention policies and notification settings.
  tags:
    - rundeck
    - database
    - backup
...
for city in cities:
    with open(f"database-backup-restore-{city}.yaml", "w") as f:
