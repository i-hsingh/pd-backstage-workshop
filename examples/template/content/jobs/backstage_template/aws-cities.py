...
kind: Template
metadata:
  name: rundeck-job-template-{city}
  title: Deploy AWS Resources ({citytitle})
  description: Use this template to deploy EC2 host resources as user{city}.
  tags:
    - rundeck
    - job
    - automation
spec:
  ...
  steps:
    - id: rundeck-execute
      ...
      input:
        jobId: 'aws-provisioning-user-{city}'
        projectName: NA
        parameters:
...
for city in cities:
    with open(f"aws-provisioning-{city}.yaml", "w") as f:
